...
image = Image.open("FBLogo.png").convert('L')
    ...
    def __init__(self, polygons=None, colors=None):
        ...
        if polygons == None:
            ...
            for i in range(self.shapes):
                # randomly selects sides of polygons
                sides = int(randint(3,6))
                coords = []
                # chooses coordinate from original image to make polygon
                point = (randint(0, image_size[0]), randint(0, image_size[1]))
                for _ in range(sides):
                    # randomly generates points for remaining polygons in given size
                    coords.append((point[0]+randint(-self.size, self.size), \
                        point[1]+randint(-self.size, self.size)))
                # Sets alpha value for polygon
...
class ImageEvolution(EvolutionaryAlgorithm):
    def __init__(self):
    ...
    def computeFitness(self, polygon):
        # Calculates mean square error for fitness value
    ...
    def crossover(self, parent1, parent2):
        ...
        # randomly selects two points for crossover
        start = randint(0, ((total-1)//2))
        end = randint(start, (total-1))
        
        # Copies genes from parent to offspring
        ...
        # Adds remaining genes from other parent
